...
class TableCanvas(Canvas):
    ...
    def do_bindings(self):
        ...
        self.bind("<Button-1>",self.handle_single_click)
        self.bind("<Double-Button-1>",self.handle_left_click)
        ...
        self.bind_all("<Control-x>", self.ctrlx)
        self.bind_all("<Delete>", self.cleartxt)
        self.bind_all("<Control-c>", self.ctrlCopy)
        self.bind_all("<Control-v>", self.ctrlPaste)
        self.bind_all("<Control-z>", self.doundo)
        self.bind_all("<Control-Shift-KeyPress-Z>", self.ctrlshiftz)
    ...
    def getundo(self,event=None):
        copy = self.model
        if len(self.undo) > 0:
            if copy != self.undo[-1]:
                self.undo.append(copy.copy())
        else:
            self.undo.append(copy.copy())
        return
    ...
    def doundo(self,event=None):
        ...
        if len(self.undo) > 0:
            if copy != self.undo[-1]:
                self.shiftz.append(copy.copy())
        else:
            self.shiftz.append(copy.copy())

        if hasattr(self, 'cellentry'):

            if self.cellentry.get():
                self.cellentry.destroy()
                row = self.getSelectedRow()
                col = self.getSelectedColumn()
                self.model.setValueAt('', row, col)
                self.redrawTable()
                delattr(self,'cellentry')

                if self.textundo[1] == 1:
                    self.undo.pop()
        elif len(self.undo)>0:

            if hasattr(self, 'cellentry'):
                self.cellentry.destroy()
            self.setModel(self.undo[-1])
            self.undo.pop()
            self.redrawTable()
        else:
            return
    ...
    def ctrlshiftz(self, event=None):
        ...
        if len(self.shiftz) > 0:

            if hasattr(self, 'cellentry'):
                self.cellentry.destroy()
            self.setModel(self.shiftz[-1])
            self.shiftz.pop()
            self.redrawTable()
        else:
            return
    ...
    def addRows(self, num=None, ID=''):
        ...
        if not num:
             return
        keys = self.model.autoAddRows(num, Identifier=ID)
        self.redrawTable()
    ...
    def deleteCells(self, rows=None, cols=None):
        self.getundo()
    ...
    def formula_Dialog(self, row, col, currformula=None):
        ...
        self.formulaFrame = Frame(width=w,height=h)
    ...
    def copyCell(self, rows, cols=None, event=None):
    ...
    def ctrlCopy(self, event=None):
        ...
        if len(self.multiplerowlist) > 1 or len(self.multiplecollist) > 1:
            ...
            for col in cols:
                for row in rows:
                    self.largeclipboard.append(copy.deepcopy(self.model.getCellRecord(row, col)))
            self.largeclipboard.append(len(rows))
            self.largeclipboard.append(len(cols))
            return
        else:
            try:
                self.largeclipboard = []
    ...
    def ctrlPaste(self, event):
        """Paste cell from internal clipboard"""
        self.getundo()
        row = self.getSelectedRow()
        col = self.getSelectedColumn()
        if len(self.largeclipboard) > 1:
            for rown in range(int(self.largeclipboard[-2])):
                for coln in range(int(self.largeclipboard[-1])):
                    for i in range(0,len(self.largeclipboard)-2):
                        try:
                            self.model.setValueAt(self.largeclipboard[i], row+rown, col+coln)
                        except:
                            continue
            self.redrawTable()
        else:
    ...
    def drawCellEntry(self, row, col, text=None):
        ...
        txtvar = StringVar()
        txtvar.set(text)
        self.textundo[0] = txtvar.get()
        self.textundo[1] = 0
        def callback(e):

            value = txtvar.get()
            if self.textundo != value:
                if self.textundo[1] == 0:
                    self.textundo[1] = 1
                    print(txtvar.get())
                    self.textundo[0] = value
                    self.getundo()
            if value == '=':
        ...
        #self.cellentry.bind('<Return>', self.getundo, add='+')
    ...
    def drawMultipleCells(self):
        ...
        rect = self.create_rectangle(x1+w/2,y1+w/2,x2,y2,
                             outline='#6e1010',width=w,activefill='red',activestipple='gray25',
                             tag='multicellrect')
    ...
    def drawTooltip(self, row, col):
        ...
        if type(text) is float or type is int:
            text = str(text)
        if text == None or text == '' or len(str(text))<=20:
            return
        ...
        rect2 = self.create_rectangle(x1,y1,x2,y2,tag='tooltip',fill='#6e1010')
        self.lift(obj)
        return
